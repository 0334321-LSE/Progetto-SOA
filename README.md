...
### Inode
To check and block the different interaction with blacklisted file the inode number of the file/directory is saved inside the monitor. Especially, the monitor mantains protected path inside a list head structure and each entry of this list is composed like: 
...
### Adding Paths
When a path is added, if it is a file, it is simply added to the blacklist. However, if it is a directory, there are two possibilities: 
...
### Defered Work
The hash evaluation of the file program path that has accessed a blacklisted file/dir and the write on the log file is done in defered work. The write must be executed after the hash evalutation, so it waits the completition of this function. 
...
``` 
The struct *work_struct* for get_log and write_log is manteined inside a custom struct **packed_work**, this permits to both the function to interact by notifing the completition.


## System calls
This solution introduces 5 system call:
...
###  User: The Client
The user can interact directly with the monitor by using an specific client. This client show a menu where the possible operations are exposed. All the interaction between user and kernel happen by using reference monitor systemcalls.
